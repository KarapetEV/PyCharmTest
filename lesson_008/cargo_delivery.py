...
class AutoLoader(Vehicle):
    ...
    def load(self):
        ...
        print('{} загружал {}'.format(self.model, self.truck))
...
for number in range(3):
    loader = AutoLoader(model='Bobcat #{}'.format(number), bucket_capacity=1000, warehouse=moscow, role='loader')
    loaders.append(loader)

for number in range(3):
    unloader = AutoLoader(model='Lonking #{}'.format(number), bucket_capacity=500, warehouse=piter, role='unloader')
    loaders.append(unloader)
